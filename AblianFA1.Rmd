...
1.The skewness of the four columns are symmetric, although they are not exactly the same. This means that there are different data but not so far from each other.
...
```
We can compute the mean, median, and sd to get the skewness with the formula 3(mean-median)/sd.
```{r}
...
Alternatively, you can use the library function if your goal is just the skewness and you don't need the mean, median, and sd.
```{r}
library(e1071) 
...
2.The females have a higher average score than the males. But a circle on the plot means that a male person scored the highest of all the people in the data.The lowest average of the females’ score is already close to the highest average scores of the males.
